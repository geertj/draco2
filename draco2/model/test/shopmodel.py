...
class StatID(IntegerAttribute):
    name = 'id'

class StatCount(IntegerAttribute):
    name = 'count'

class Stat(Entity):
    name = 'stat'
    attributes = [StatID, StatCount]
    primary_key = [StatID]
...
class OrderView(View):
    ...
    query = """
            ...
            FROM shopmodel.orders
                 INNER JOIN shopmodel.user_order
                        ON shopmodel.user_order.order_id = shopmodel.orders.id
                 INNER JOIN shopmodel.users
                        ON shopmodel.users.id = shopmodel.user_order.user_id
            """
...
class ShopModel(Model):
    name = 'shopmodel'
    version = 1
    entities = [User, Product, Order, Stat]
